...
let turtles = document.querySelectorAll(".tur1 , .tur2");
let turtleData = [];
...
function initializeCats() {
  catData = Array.from(cats).map((cat) => {
    ...
    // const speed = (1.8 + Math.random()) * getScale();
...
function initializeTurtles() {
  turtleData = Array.from(turtles).map((tur, index) => {
    ...
    if (index === 0) {
      ...
      startX = container.clientWidth - tur.offsetWidth; // right side
      direction = -1;
    }
