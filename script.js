...
let hasLeftGate = false; // Track if Flicky has moved away from the gate
...
//   if (w < 600) return 0.9;      // mobile (faster feel)
//   if (w < 1024) return 1.1;     // tablet
//   if (w < 1500) return 1.3;     // small laptop
//   return 1.6;                  // BIG screens (1500px+)
// }
...
function getScale() {
  ...
  return 0.8; // IMPORTANT: cap big screens
}
...
// const moveSpeed = 7 * getScale();
// const jumpPower = -21 ;


// const moveSpeed = 5 * getScale();
// const jumpPower = -18 * getScale();
// const gravity = 0.9 * getScale();
...
function setFlickyAtGate() {
  ...
  // Position Flicky beside the gate (to the right), on the ground
  ...
  velocityY = 0;
  isJumping = false;
  currentPlatform = "ground"; // She starts on the ground
}
...
function loseLife() {
  ...
  if (lives > 0) {
    ...
    // OPTIONAL: after 2 sec show buttons or reload
    setTimeout(() => {
      // location.reload(); // if you want auto restart
    }, 2000);
  }
}
...
function gameLoop() {
  ...
  // Track if Flicky has left the gate
  if (gate && !hasLeftGate) {
  ...
  // Update cat movement and handle collisions
  ...
  // Win condition: must have left gate, collected all babies, and then returned to gate
